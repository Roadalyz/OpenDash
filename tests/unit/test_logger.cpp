...
{
    constexpr int TEST_NUMBER = 42;
    constexpr double TEST_PI = 3.14;
    
    auto logger = Logger::get_default();
    ASSERT_NE(logger, nullptr);

    // These should not crash
    logger->info("Formatted message with number: {}", TEST_NUMBER);
    logger->debug("Multiple args: {} and {}", "hello", TEST_PI);
}
