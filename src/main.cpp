...
constexpr uint32_t FRAME_PROGRESS_LOG_INTERVAL = 100;  // Log every 100 frames
constexpr uint32_t DETAILED_PROGRESS_LOG_INTERVAL = 1000;  // Detailed log every 1000 frames
...
class DashcamApplication
{
  public:
    ...
    int run()
    {
        ...
        while (!g_shutdown_requested.load() && frame_count < MAX_FRAMES_PER_SESSION)
        {
            ...
            if (frame_count % FRAME_PROGRESS_LOG_INTERVAL == 0)
            {
        ...
        {
            LOG_WARNING("Reached maximum frames per session ({}), stopping",
  ...
  private:
    ...
    void process_frame(uint32_t frame_number)
    {
        ...
        if (frame_number % DETAILED_PROGRESS_LOG_INTERVAL == 0)
        {
