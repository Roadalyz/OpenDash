...
bool Logger::initialize(LogLevel default_level)
{
    ...
    try
    {
        ...
        default_config.max_file_size_bytes = LoggerConfig::DEFAULT_MAX_FILE_SIZE_MB * LoggerConfig::BYTES_PER_MB;
        default_config.max_files = LoggerConfig::DEFAULT_MAX_FILES;
    ...
    try
    {
        ...
        if (config.enable_file && !config.file_path.empty())
        {
            ...
            auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
                config.file_path, config.max_file_size_bytes, config.max_files);
        ...
        spdlog_logger->flush_on(
            spdlog::level::info);  // Flush on info level and above for immediate writing
